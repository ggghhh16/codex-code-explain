...
独立开发的第三方 VS Code 扩展，非 OpenAI / Microsoft 官方产品。当前发布版本：**0.2.1**。
...
2. 从 Releases 下载 `codex-code-explain-0.2.1.vsix`，然后 VS Code → 扩展 → 右上角菜单 → **从 VSIX 安装**；如提示重载，执行重载窗口。
...
讲解要求对应学习协议中的来源、命名权、实际形状、参数约束和对象传递过程。它是按需代码查阅工具，不恢复课程、修改任务栈、创建 pending、认定掌握、写学习地图或发送 Telegram。项目关键机制仍需主教学主动说明。
