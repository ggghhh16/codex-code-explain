...
但“只在当前项目遇到”不总等于“不可迁移”。函数引用与调用、对象共享、回调时机、数据形状等机制可能在许多项目重复出现。插件先说明当前具体行为，再用一句话区分项目选择和可迁移规则，避免把所有陌生名称都升级成课程，也避免省略关键机制。

主教学仍需主动提供学习协议规定的来源与过程说明。插件减少临时追问成本，不能成为允许主教学遗漏来源的理由。
...
**学习协议：** 提示词采用来源、命名权、形状、当前参数约束、实际对象过程等内容约束。插件没有 confirmed/pending 权限，也不认定用户掌握，不修改路线、任务栈、学习地图、陌生知识块或推送。笔记只是用户主动保存的代码查阅材料。
